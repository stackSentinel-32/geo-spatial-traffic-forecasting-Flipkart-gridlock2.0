...
"""
=============================================================================
  Flipkart GriLock 2.0 - ULTRA Pipeline (v3)
  Target: 93-94 Online Score
  
  NEW vs v2:
    1.  Temporal lag features  -- demand 15-min ago / 1-day ago per geohash
    2.  Spatial KNN neighbors  -- avg demand of nearest geohash cells
    3.  Day-recency weighting  -- recent day=49 rows get 2x sample weight
    4.  Yeo-Johnson transform  -- better than log1p for skewed distributions
    5.  CatBoost Optuna HPO    -- tune the best model (was skipped in v2)
    6.  HistGradientBoosting   -- sklearn's fast GBM as 3rd model
    7.  XGBoost removed        -- consistently hurt ensemble (neg weight)
    8.  Pseudo-labeling        -- semi-supervised boost using test preds
    9.  Optuna blend weights   -- non-negative optimal weights, replaces Ridge
   10.  Per-geohash variance    -- demand volatility as uncertainty feature
=============================================================================
"""
...
print("=" * 65)
print("  FLIPKART GRILOCK 2.0 - ULTRA PIPELINE (v3)")
print("  Target: 93-94 Online Score")
print("=" * 65)
...
#    For test at (geohash, day=49, 2:15) -> look up train demand at:
#      a) same geohash, day=49, 2:0  (15-min lag)
#      b) same geohash, day=48, 2:15 (1-day lag, same time)
#      c) same geohash, day=48, 2:0  (1-day + 15-min lag)
#      d) same geo4,    day=49, 2:0  (area-level 15-min lag)
# ─────────────────────────────────────────────────────────────────────────────
print("\n[4/11] Building temporal lag demand features...")

# Build lookup tables from training data
demand_lookup_gh   = train.set_index(['geohash','day','minutes'])['demand']
demand_lookup_geo4 = train.groupby(['geo4','day','minutes'])['demand'].mean()
demand_lookup_geo3 = train.groupby(['geo3','day','minutes'])['demand'].mean()

# All unique (day, minutes) combos in training
...
print(f"       Train time combos: {[(int(d),int(m)) for d,m in train_times]}")

# Find the most recent training timestep before test time
recent_lags = []
for d, m in train_times:
    # Only use time steps that could be "before" test time
...
def build_lag_features(df, is_test=True):
    df = df.copy()
    for i, (lag_day, lag_min, delta) in enumerate(recent_lags[:4]):
        col = f'lag_{i+1}_gh'  # geohash-level lag
        df[col] = df['geohash'].map(
            demand_lookup_gh.xs((lag_day, lag_min), level=['day','minutes'])
            if (lag_day, lag_min) in demand_lookup_gh.index.droplevel(0).unique()
            else pd.Series(dtype=float)
        )
        # geo4-level lag (more coverage for unseen geohashes)
        col_g4 = f'lag_{i+1}_geo4'
        if (lag_day, lag_min) in demand_lookup_geo4.index.droplevel(0).unique() if hasattr(demand_lookup_geo4.index, 'droplevel') else True:
            try:
                map_g4 = demand_lookup_geo4.xs((lag_day, lag_min), level=['day','minutes'])
                df[col_g4] = df['geo4'].map(map_g4)
            except:
                df[col_g4] = np.nan
        else:
            df[col_g4] = np.nan
...
def add_lag_features(df):
    ...
    for i, (lag_day, lag_min, delta) in enumerate(recent_lags[:4]):
        ...
        try:
            subset = train[(train['day']==lag_day) & (train['minutes']==lag_min)][['geohash','demand']]
        ...
        # geo4-level lookup
        try:
            subset_g4 = train[(train['day']==lag_day) & (train['minutes']==lag_min)].groupby('geo4')['demand'].mean()
        ...
        # geo3-level lookup
        try:
            subset_g3 = train[(train['day']==lag_day) & (train['minutes']==lag_min)].groupby('geo3')['demand'].mean()
            df[f'lag_{i+1}_geo3'] = df['geo3'].map(subset_g3)
        except:
            df[f'lag_{i+1}_geo3'] = np.nan

    # Fill lags with chain: geo3 → geo4 → global mean
    global_mean_raw = train['demand'].mean()
...
#    For each geohash, find K=8 spatially nearest geohashes and
#    use their demand statistics as features
# ─────────────────────────────────────────────────────────────────────────────
print("\n[5/11] Building spatial KNN neighbor demand features...")

# Get unique geohash locations with mean demand from training
...
# Build KNN on training geohash coordinates
...
def get_knn_features(df):
    ...
    for i in range(len(df)):
        ...
        mask = d_list > 1e-8
        if mask.sum() == 0:
            idx_list = idxs[i][1:K_NEIGHBORS+1]
            d_list   = dists[i][1:K_NEIGHBORS+1]
        else:
            idx_list = idx_list[mask][:K_NEIGHBORS]
            d_list   = d_list[mask][:K_NEIGHBORS]
...
train = get_knn_features(train)
test  = get_knn_features(test)
...
# Apply Yeo-Johnson transform first, then do TE
...
for cols in TE_COLS:
    ...
    for tr_i, val_i in kf_te.split(train):
        ...
        stats['te'] = (stats['count']*stats['mean']+SMOOTH_ALPHA*global_y)/(stats['count']+SMOOTH_ALPHA)
        keys = fv[list(cols)]
        if len(cols)==1:
    ...
    stats_all['te'] = (stats_all['count']*stats_all['mean']+SMOOTH_ALPHA*global_y)/(stats_all['count']+SMOOTH_ALPHA)
    if len(cols)==1:
...
# ─────────────────────────────────────────────────────────────────────────────
# 8. OPTUNA HPO — LightGBM (50 trials)
# ─────────────────────────────────────────────────────────────────────────────
print("\n[8/11] Optuna HPO for LightGBM (50 trials)...")
N_HPO_FOLDS = 3
...
# ─────────────────────────────────────────────────────────────────────────────
# 9. OPTUNA HPO — CatBoost (30 trials, our best v2 model)
# ─────────────────────────────────────────────────────────────────────────────
print("\n[9/11] Optuna HPO for CatBoost (30 trials)...")
...
print(f"\n[10/11] Training 3 models — {N_FOLDS}-Fold CV...")
...
for fold,(ti,vi) in enumerate(kf.split(X,y),1):
    ...
    print(f"  Fold {fold:02d}/{N_FOLDS} -> RMSE: {r:.5f}  R2: {r2:.4f}  iter={m.best_iteration_}")
...
for fold,(ti,vi) in enumerate(kf.split(X,y),1):
    ...
    hgb_test   += m.predict(X_test) / N_FOLDS
    r, r2 = rmse_orig(y[vi], hgb_oof[vi])
    print(f"  Fold {fold:02d}/{N_FOLDS} -> RMSE: {r:.5f}  R2: {r2:.4f}  iter={m.n_iter_}")
...
# Use confident test predictions as additional training data
# ─────────────────────────────────────────────────────────────────────────────
...
# Use 80% for train, 20% val for early stopping
...
submission.to_csv('submit/sample_submission_v3.csv', index=False)
...
print(f"    [+] Spatial KNN neighbor demand (K={K_NEIGHBORS})")
print(f"    [+] Yeo-Johnson target transform")
print(f"    [+] CatBoost Optuna HPO (30 trials)")
print(f"    [+] LightGBM Optuna HPO (50 trials)")
...
print(f"    [+] {len(FEATURES)} total features (vs 56 in v2)")
print()
...
print(f"  Submission file : submit/sample_submission_v3.csv ({len(submission)} rows)")
